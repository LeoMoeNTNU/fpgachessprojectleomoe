...
# I assume that if castling is allowed, then that means that there is a tower there that hasn't moved yet. 
This may very well be a bad idea, I am not sure. 

# In the checkbybishop and the similar ones, I have a bit called valid. 
This one indicates if the piece marked is the same color as the color we are inputting. 
